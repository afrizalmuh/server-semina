...
const deletePayments = async (req) => {
  ...
  if (!result) throw new NotFoundError(`Tidak ada tipe pembayaran degan id: ${id}`)
...
const checkingPayments = async (id) => {
  ...
  if (!result)
    throw new NotFoundError(`Tiadk ada tipe pembayaran dengan id: ${id}`)
